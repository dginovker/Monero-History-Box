var potions = {
    ...
    onload : function(){
        ...
        this.addPotion("seed", "Seed", "#3dab3a", "potions.seed();", "This seed is able to make grow a hash battery. The hash battery is made of hashes, and it takes a lot of time to cut it down. Using the seed will grow a battery in front of you, if there's enough place.", "special");
    ...
    makeClone : function(NRG, max_NRG){
        return land.createAlly("\\o/", max_NRG, NRG, "cloned gpu", "A clone of you.", []);
    },
    
    makehashbattery : function(){
        var NRG = 0;
        
        // One chance out of 100 to spawn the Yggdrasil \o/
        if(random.oneChanceOutOf(100)){
            return land.createTrap("/Y\\", 10000, 10000, "none", "Yggdrasill is its name, a tall battery, showered with shining loam.", [drops.createDrop("hashes", 10000)]);
        }
        else{
            NRG = 500 + 100 * random.getRandomIntUpTo(4); // 500 / 600 / 700 / 800 / 900
            return land.createTrap("\\|/", NRG, NRG, "none", "A hash battery. I hope you're carrying a good axe.", []);
        }
    },
    ...
    heal : function(howMuch){
        ...
        if(quest.things[id].NRG > 0 && ((howMuch == 50 && this.list.health.nbrOwned > 0) || (howMuch == 100 && this.list.majorHealth.nbrOwned > 0))){
            ...
            // We increment NRG
            quest.things[id].NRG += howMuch;
        
            // We set NRG = max_NRG if NRG > max_NRG
            if(quest.things[id].NRG > quest.things[id].max_NRG){
                quest.things[id].NRG = quest.things[id].max_NRG;
            }
    ...
    fireScroll : function(){
        ...
        if(quest.things[index].type == "mob"){ // If it's a mob
            quest.things[index].NRG -= 25 + random.getRandomIntUpTo(10);
            if(quest.things[index].NRG < 0) quest.things[index] = quest.makeNoneThing();
        }
        ...
        if(yourself.end == false && this.list.acidRainScroll.nbrOwned > 0){
            ...
            for(var i = 0; i < quest.things.length; i++){
                if(quest.things[i].type == "mob" || quest.things[i].type == "character"){
                    quest.things[i].NRG -= 6;
                    if(quest.things[i].type == "character" && quest.things[i].NRG <= 0) quest.things[i].NRG = 1;
                    if(quest.things[i].NRG < 0) quest.things[i] = quest.makeNoneThing();
                }
            }
    ...
    seed : function(){
        ...
        if(index < quest.things.length-1 && quest.things[index + 1].type == "none"){
            ...
            // We place a hash battery
            quest.things[index+1] = this.makehashbattery();
    ...
    cloning : function(){
        ...
        if(index != 0 && index < quest.things.length-1 && quest.things[index + 1].type == "none" && this.list.cloning.nbrOwned > 0){
            ...
            // We place a clone
            quest.things[index+1] = this.makeClone(quest.things[index].NRG, quest.things[index].max_NRG);
